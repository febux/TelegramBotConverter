...
from src.bot__app.bot_funcs.handle_callback import callback_text
...
from app import dp, api, bot, phrases


@dp.message_handler(commands=['start'])
async def start_processing(message: Message):
    await handle_start(bot=bot, message=message, api=api, phrases=phrases)


@dp.message_handler(commands=['help'])
async def help_processing(message: Message):
    await handle_help(bot=bot, message=message, api=api, phrases=phrases)


@dp.message_handler(content_types=['text'])
async def message_text_processing(message: Message):
    await handle_text(bot=bot, message=message, api=api, phrases=phrases)


@dp.callback_query_handler(lambda callback_query: True)
async def callback_inline(callback):
    await callback_text(bot=bot, callback=callback, api=api, phrases=phrases)
